...
#define REF_PERFORMANCE 46796.3 //UPDATE THIS WITH YOUR REFERENCE PERFORMANCE (see console after 2k frames)
...
void Game::init()
{
    ...
    {
        tanks.push_back(Tank(start_blue_x + ((i % max_rows) * spacing), 
    ...
    for (int i = 0; i < NUM_TANKS_RED; i++)
    {
        tanks.push_back(Tank(
                start_red_x + ((i % max_rows) * spacing), 
...
Tank& Game::find_closest_enemy(Tank& current_tank)
{
    ...
    {
        if (tanks.at(i).alignment != current_tank.alignment && tanks.at(i).active)
        {
...
void Game::update(float deltaTime)
{
    ...
    for (Rocket& rocket : rockets)
    {
        ...
        for (Tank& tank : tanks)
        {
            if (tank.active && (tank.alignment != rocket.alignment) && rocket.intersects(tank.position, tank.collision_radius))
            {
...
void Game::draw()
{
    ...
    auto health_bar_future = threadpool.enqueue(draw_health_bars);
    screen->clear(0);
    ...
    health_bar_future.wait();
}

int Tmpl8::Game::draw_health_bars() {
...
{
    //PROFILE_FUNCTION();
    InstrumentationTimer timer("Game::tick");
    if (!lock_update)
    {
        update(deltaTime);
    }
